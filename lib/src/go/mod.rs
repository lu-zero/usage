...
/// Generate explicitly typed fields, keyed by the generated flag/argument constants.
///
/// Unlike [`generate`], this rejects unknown keys and entries without text values.
/// Types are Go-specific configuration, not inferred from usage placeholder names.
pub fn generate_with_types(
    ...
    if field_types.is_empty() {
        return Ok(generate(spec, opts));
    }
    let mut probe = Emitter::new(spec, opts);
    let mut commands = Vec::new();
    probe.collect(&spec.cmd, &[], true, &mut commands);
    types::validate(&commands, field_types)?;
    let mut emitter = Emitter::new(spec, opts);
    emitter.field_types = field_types.clone();
    Ok(emitter.run())
}
...
impl<'a> Emitter<'a> {
    ...
    fn run(mut self) -> String {
        ...
        structs::emit(&mut self.out, &commands, &self.field_types);
